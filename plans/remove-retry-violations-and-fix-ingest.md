...
1. Uncapped raw HTML (plus capped markdown) was sent inline per page → Mongo BSON **16 MiB** document overflows and/or API/repo request-size mismatch.
...
| Rule | Decision |
|------|----------|
| Truncation | **Prohibited** for `html` and `markdown`. Never shorten content to fit. |
| Markdown | Always the preferred stored body when present. Crawler may send `markdown` (+ `title` / `excerpt`). |
...
| When HTML would exceed budget | Set `html` to `null`, persist the rest of the page (including markdown when present), and record `htmlOmittedBytes` + reason in structured logs / optional response metadata. This is an **explicit contract path**, not a silent fallback. |
